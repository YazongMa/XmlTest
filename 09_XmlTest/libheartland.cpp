...
static ST_HSOAP_INIT hsoapHeaderData;
...
unsigned short HeartlandSoap::HeartlandPackProcessEntry(SoapElementRef startNode, ST_HSOAP_REQUEST *hsoapData)
{
    ...
    if (txnType == tt_DEBIT_SALE || txnType == tt_DEBIT_RETURN)
    {
        HeartlandAddSubElement(startNode, NULL_STRING, NULL_STRING, TAG_NAME_ACCOUNT_TYPE, hsoapData->szAccountType, sizeof(hsoapData->szAccountType), false);
        HeartlandAddSubElement(startNode, NULL_STRING, NULL_STRING, TAG_NAME_POS_SEQUENCE_NBR, hsoapData->szPosSequenceNbr, sizeof(hsoapData->szPosSequenceNbr), false);
    }
...
void HeartlandSoap::HeartlandPackElement_HlData(SoapElementRef node, ST_HSOAP_REQUEST *pData)
{
    ...
    if (pData->nTransactionType == tt_DEBIT_SALE || pData->nTransactionType == tt_DEBIT_RETURN || pData->nTransactionType == tt_DEBIT_REVERSAL)
    {
        ...
        if (pData->nTransactionInterface != ti_SWIPE)
        {
